    ...
    <AppLayout>
        <div class="page-head">
            <div class="min-w-0">
                <div class="page-head__title">
                    <Box :size="18" :stroke-width="2" class="text-[color:var(--color-accent)]" />
                    Block <span class="num">#{{ fmtInt(block.height) }}</span>
                    <span class="pill pill--ok">{{ fmtInt(confirmations) }} confirmations</span>
                </div>
                <div class="page-head__sub">{{ block.hash }}</div>
            </div>
            <div class="flex items-center gap-2">
                <Link :href="`/block/${block.height - 1}`" class="btn-ghost"><ChevronLeft :size="14" /> Prev</Link>
                <Link
                    v-if="block.height < tip"
                    :href="`/block/${block.height + 1}`"
                    class="btn-ghost"
                >Next <ChevronRight :size="14" /></Link>
            </div>
        </div>
        ...
        <div class="section mb-4">
            ...
            <div class="detail-grid">
                <div class="detail-row">
                    ...
                    <div class="detail-value mono">{{ fmtInt(block.height) }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Timestamp</div>
                    <div class="detail-value">{{ isoTime(block.timestamp) }} <span class="text-[color:var(--color-fg-dim)]">· {{ block.age }}</span></div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Hash</div>
                    <div class="detail-value mono text-xs break-all">{{ block.hash }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Parent hash</div>
                    <div class="detail-value">
                        <Link :href="`/block/${block.height - 1}`" class="hash-link">
                ...
                <div class="detail-row">
                    ...
                    <div class="detail-value mono">{{ fmtInt(block.tx_count) }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Gas used</div>
                    <div class="detail-value mono">
                        {{ fmtInt(block.gas_used) }}
                        <span class="text-[color:var(--color-fg-dim)]">/ {{ fmtInt(block.gas_limit) }} ({{ block.gas_used_pct }}%)</span>
                    </div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Base fee</div>
                    <div class="detail-value mono">{{ fmtGwei(block.base_fee_gwei) }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Block reward</div>
                    <div class="detail-value mono">{{ fmtEth(block.reward_eth) }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Size</div>
                    <div class="detail-value mono">{{ fmtBytes(block.size_bytes) }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="section">
            <div class="section__head">
                ...
                <span class="section__sub">{{ block.transactions.length }} of {{ fmtInt(block.tx_count) }}</span>
            </div>
            <div class="data-list">
                <Link
                ...
                >
                    ...
                    <div class="field">
                        ...
                        <div class="field__sub"><span class="method">{{ t.method }}</span></div>
                    </div>
                    <div class="field">
